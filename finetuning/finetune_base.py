...
# -*- coding: utf-8 -*-
"""
Fine-tuning Script for VA-RoBERTa BASE Model
Fine-tunes pretrained BASE model on language-specific data and generates predictions
Separate file for BASE model only
"""
...
# Import model from pretraining - BASE model specific
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))

# Import model classes from BASE pretraining script
from pretraining.pretrain_base import VARoBERTaModel, VADataset, process_data, load_jsonl, set_seed

# Import BASE model config
from config_base import CFG_BASE as CFG
...
def main():
    ...
    # Using BASE model config (already imported)
    print(f"  Using BASE model config: {CFG.model_name}")
