...
# -*- coding: utf-8 -*-
"""
Fine-tuning Script for VA-RoBERTa Model
Fine-tunes pretrained model on English data and generates predictions
"""
...
# Import model from pretraining - will use model-specific configs
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))

# Import model classes (same for all models)
from pretraining.pretrain import VARoBERTaModel, VADataset, process_data, load_jsonl, set_seed

# Model-specific configs will be loaded based on model_name argument

# Configuration
class CFG:
    max_len = 192
    batch_size = 4  # Set to 4 for all models
    epochs = 15  # Increased from 5 to allow more training (early stopping will prevent overfitting)
    lr = 1.5e-5
    dropout = 0.1
    fc_dropout = 0.3
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    seed = 42
    early_stopping_patience = 50  # Very high patience - train until good RMSE is achieved
    gradient_accumulation_steps = 1  # No gradient accumulation needed with batch_size=4
    use_float16 = True  # Use mixed precision training for memory efficiency

set_seed(CFG.seed)

def train_epoch(model, dataloader, criterion, optimizer, device, gradient_accumulation_steps=1, scaler=None):
    ...
    autocast_context = torch.cuda.amp.autocast() if use_amp else None
...
def main():
    ...
    # Load model-specific config based on model name
    if 'large' in base_model_name.lower():
        ...
        print(f"  Batch size: {CFG.batch_size}, Float16: {CFG.use_float16}, Patience: {CFG.early_stopping_patience}")
    elif 'xl' in base_model_name.lower():
        from config_xl import CFG_XL as CFG
        CFG.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"  Using XL model config: {CFG.model_name}")
        print(f"  Batch size: {CFG.batch_size}, Float16: {CFG.use_float16}, Patience: {CFG.early_stopping_patience}")
    elif 'mdeberta' in base_model_name.lower() or 'deberta' in base_model_name.lower():
        from config_mdeberta import CFG_MDEBERTA as CFG
        CFG.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"  Using mDeBERTa model config: {CFG.model_name}")
        print(f"  Batch size: {CFG.batch_size}, Float16: {CFG.use_float16}, Patience: {CFG.early_stopping_patience}")
    else:
        print("  Warning: Unknown model, using default config")
        CFG.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            ...
            if args.skip_training:
                ...
                with open(results_log, 'w', encoding='utf-8') as log_file:
                    ...
                    for epoch in range(CFG.epochs):
                        ...
                        train_loss = train_epoch(model, train_loader, criterion, optimizer, CFG.device, CFG.gradient_accumulation_steps, scaler)
